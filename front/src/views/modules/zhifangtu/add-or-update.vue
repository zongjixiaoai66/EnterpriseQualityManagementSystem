    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="gongzuorenyuanId" name="gongzuorenyuanId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="输出编号" prop="zhifangtuUuidNumber">
                       <el-input v-model="ruleForm.zhifangtuUuidNumber"
                                 placeholder="输出编号" clearable  :readonly="ro.zhifangtuUuidNumber"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="输出编号" prop="zhifangtuUuidNumber">
                           <el-input v-model="ruleForm.zhifangtuUuidNumber"
                                     placeholder="输出编号" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="输出标题" prop="zhifangtuName">
                       <el-input v-model="ruleForm.zhifangtuName"
                                 placeholder="输出标题" clearable  :readonly="ro.zhifangtuName"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="输出标题" prop="zhifangtuName">
                           <el-input v-model="ruleForm.zhifangtuName"
                                     placeholder="输出标题" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="直方图输出类型" prop="zhifangtuTypes">
                        <el-select v-model="ruleForm.zhifangtuTypes" :disabled="ro.zhifangtuTypes" placeholder="请选择直方图输出类型">
                            <el-option
                                v-for="(item,index) in zhifangtuTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="直方图输出类型" prop="zhifangtuValue">
                        <el-input v-model="ruleForm.zhifangtuValue"
                            placeholder="直方图输出类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12">
                    <el-form-item class="upload" v-if="type!='info' && !ro.zhifangtuFile" label="输出文件" prop="zhifangtuFile">
                        <file-upload
                                tip="点击上传输出文件"
                                action="file/upload"
                                :limit="3"
                                :multiple="true"
                                :fileUrls="ruleForm.zhifangtuFile?$base.url+ruleForm.zhifangtuFile:''"
                                @change="zhifangtuFileUploadChange"
                        ></file-upload>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.zhifangtuFile" label="输出文件" prop="zhifangtuFile">
                            <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+ruleForm.zhifangtuFile"  >输出文件下载</a>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12">
                    <el-form-item v-if="type!='info' && !ro.zhifangtuTime"  class="input" label="输出时间" prop="zhifangtuTime">
                        <el-date-picker
                                value-format="yyyy-MM-dd HH:mm:ss"
                                v-model="ruleForm.zhifangtuTime"
                                type="datetime"
                                placeholder="输出时间"
                                :disabled="ro.zhifangtuTime">
                        </el-date-picker>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.zhifangtuTime" label="输出时间" prop="zhifangtuTime">
                            <span v-html="ruleForm.zhifangtuTime"></span>
                        </el-form-item>
                    </div>
                </el-col>

                <el-col :span="24">
                    <el-form-item v-if="type!='info' && !ro.zhifangtuContent"  label="输出备注" prop="zhifangtuContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.zhifangtuContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.zhifangtuContent" label="输出备注" prop="zhifangtuContent">
                            <span v-html="ruleForm.zhifangtuContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    ...
                    zhifangtuUuidNumber: true,
                    zhifangtuName: false,
                    zhifangtuTypes: false,
                    zhifangtuFile: false,
                    zhifangtuTime: false,
                    zhifangtuContent: false,
                    insertTime: false,
                },
                ruleForm: {
                    gongzuorenyuanId: '',
                    zhifangtuUuidNumber: new Date().getTime(),
                    zhifangtuName: '',
                    zhifangtuTypes: '',
                    zhifangtuFile: '',
                    zhifangtuTime: '',
                ...
                rules: {
                   ...
                   zhifangtuUuidNumber: [
                              { required: true, message: '输出编号不能为空', trigger: 'blur' },
                          ],
                   zhifangtuName: [
                              { required: true, message: '输出标题不能为空', trigger: 'blur' },
                          ],
                   zhifangtuTypes: [
                              { required: true, message: '直方图输出类型不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
                   ...
                   zhifangtuFile: [
                              { required: true, message: '输出文件不能为空', trigger: 'blur' },
                          ],
                   zhifangtuTime: [
                              { required: true, message: '输出时间不能为空', trigger: 'blur' },
                          ],
                   zhifangtuContent: [
                              { required: true, message: '输出备注不能为空', trigger: 'blur' },
                          ],
                   insertTime: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=zhifangtu_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.zhifangtuTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `zhifangtu/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        _this.ruleForm = data.data;
                        _this.gongzuorenyuanChange(data.data.gongzuorenyuanId)
                        _this.ruleForm.zhifangtuContent = _this.ruleForm.zhifangtuContent.replaceAll("src=\"upload/","src=\""+this.$base.url+"upload/");
                    } else {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.ruleForm.zhifangtuContent = this.ruleForm.zhifangtuContent.replaceAll(this.$base.url,"");
                        this.$http({
                            url:`zhifangtu/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.zhifangtuCrossAddOrUpdateFlag = false;
                                        this.parent.search();
            ...
            //图片
            zhifangtuFileUploadChange(fileUrls){
                this.ruleForm.zhifangtuFile = fileUrls;
                this.addEditUploadStyleChange()
            },
