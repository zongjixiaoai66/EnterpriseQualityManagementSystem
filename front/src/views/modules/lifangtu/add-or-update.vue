    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="gongzuorenyuanId" name="gongzuorenyuanId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="输出编号" prop="lifangtuUuidNumber">
                       <el-input v-model="ruleForm.lifangtuUuidNumber"
                                 placeholder="输出编号" clearable  :readonly="ro.lifangtuUuidNumber"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="输出编号" prop="lifangtuUuidNumber">
                           <el-input v-model="ruleForm.lifangtuUuidNumber"
               ...
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="输出标题" prop="lifangtuName">
                       <el-input v-model="ruleForm.lifangtuName"
                                 placeholder="输出标题" clearable  :readonly="ro.lifangtuName"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="输出标题" prop="lifangtuName">
                           <el-input v-model="ruleForm.lifangtuName"
                ...
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="立方图输出类型" prop="lifangtuTypes">
                        <el-select v-model="ruleForm.lifangtuTypes" :disabled="ro.lifangtuTypes" placeholder="请选择立方图输出类型">
                            <el-option
                                v-for="(item,index) in lifangtuTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="立方图输出类型" prop="lifangtuValue">
                        <el-input v-model="ruleForm.lifangtuValue"
                            placeholder="立方图输出类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12">
                    <el-form-item class="upload" v-if="type!='info' && !ro.lifangtuFile" label="输出文件" prop="lifangtuFile">
                        <file-upload
                                tip="点击上传输出文件"
                                action="file/upload"
                                :limit="3"
                                :multiple="true"
                                :fileUrls="ruleForm.lifangtuFile?$base.url+ruleForm.lifangtuFile:''"
                                @change="lifangtuFileUploadChange"
                        ></file-upload>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.lifangtuFile" label="输出文件" prop="lifangtuFile">
                            <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+ruleForm.lifangtuFile"  >输出文件下载</a>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12">
                    <el-form-item v-if="type!='info' && !ro.lifangtuTime"  class="input" label="输出时间" prop="lifangtuTime">
                        <el-date-picker
                                value-format="yyyy-MM-dd HH:mm:ss"
                                v-model="ruleForm.lifangtuTime"
                                type="datetime"
                                placeholder="输出时间"
                                :disabled="ro.lifangtuTime">
                        </el-date-picker>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.lifangtuTime" label="输出时间" prop="lifangtuTime">
                            <span v-html="ruleForm.lifangtuTime"></span>
                        </el-form-item>
                    </div>
                </el-col>

                <el-col :span="24">
                    <el-form-item v-if="type!='info' && !ro.lifangtuContent"  label="输出备注" prop="lifangtuContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.lifangtuContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.lifangtuContent" label="输出备注" prop="lifangtuContent">
                            <span v-html="ruleForm.lifangtuContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    ...
                    lifangtuUuidNumber: true,
                    lifangtuName: false,
                    lifangtuTypes: false,
                    lifangtuFile: false,
                    lifangtuTime: false,
                    lifangtuContent: false,
                    insertTime: false,
                },
                ruleForm: {
                    gongzuorenyuanId: '',
                    lifangtuUuidNumber: new Date().getTime(),
                    lifangtuName: '',
                    lifangtuTypes: '',
                    lifangtuFile: '',
                    lifangtuTime: '',
                ...
                rules: {
                   ...
                   lifangtuTypes: [
                              { required: true, message: '立方图输出类型不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=lifangtu_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.lifangtuTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `lifangtu/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        _this.ruleForm = data.data;
                        _this.gongzuorenyuanChange(data.data.gongzuorenyuanId)
                        _this.ruleForm.lifangtuContent = _this.ruleForm.lifangtuContent.replaceAll("src=\"upload/","src=\""+this.$base.url+"upload/");
                    } else {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.ruleForm.lifangtuContent = this.ruleForm.lifangtuContent.replaceAll(this.$base.url,"");
                        this.$http({
                            url:`lifangtu/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.lifangtuCrossAddOrUpdateFlag = false;
                                        this.parent.search();
            ...
            //图片
            lifangtuFileUploadChange(fileUrls){
                this.ruleForm.lifangtuFile = fileUrls;
                this.addEditUploadStyleChange()
            },
